...
void USB_AudioRecorderGetBuffer(uint8_t *buffer, uint32_t size)
{
    uint32_t i, j;
    uint8_t valid_data;
    uint32_t real_data = AUDIO_IN_FORMAT_SIZE * DEMO_AUDIO_INTPUT_CHANNEL_COUNT;

    /* copy audio wav data from flash to buffer */
    for (i = 0U; i < size; i += AUDIO_IN_FORMAT_SIZE * AUDIO_IN_FORMAT_CHANNELS)
    {
        valid_data = real_data;
        for (j = 0U; j < AUDIO_IN_FORMAT_SIZE * AUDIO_IN_FORMAT_CHANNELS; j++)
        {
            if (audioPosition > (BUFFER_SIZE * BUFFER_NUM - 1U))
            {
                audioPosition = 0U;
            }
            *(buffer + i + j) = s_buffer[audioPosition];
            audioPosition++;
            valid_data--;
            if (0U == valid_data)
            {
                audioPosition -= real_data;
                valid_data = real_data;
            }
        }
        audioPosition += real_data;
