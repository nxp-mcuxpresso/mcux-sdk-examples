...
#define APP_INQUIRY_LENGTH        (10U) /* 10 * 1.28 Sec */
#define APP_INQUIRY_NUM_RESPONSES (20U)
...
static void br_discovery_complete(struct bt_br_discovery_result *results, size_t count)
{
    ...
    for (index = 0; index < count; ++index)
    {
        ...
        {
            if (results[index].rssi > max_rssi)
            {
                max_rssi = results[index].rssi;
