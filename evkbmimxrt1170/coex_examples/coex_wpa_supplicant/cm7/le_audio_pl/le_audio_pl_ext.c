...
#if defined(LE_AUDIO_ENABLE_PRINTS_FOR_STREAMING) && (LE_AUDIO_ENABLE_PRINTS_FOR_STREAMING == 1)
        PRINTF("codec i/f starts now at %d(%08u),divisor %d\n", sink_sync->sink_sync_counter, sink_sync->sink_capture1_val,sink_sync->sink_bclk_divisior);
#else
        PRINTF ("sink starts!");
...
UINT32 le_audio_pl_get_sdu_timestamp (UINT32 seq_num)
{
	...
	{
		tx_samples = 0;
		/*schedule SDU to next iso-event to avoid anchor point overlaps and null-pdu tx issue*/
