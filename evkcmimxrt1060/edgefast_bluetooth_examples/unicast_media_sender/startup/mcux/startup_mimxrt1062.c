...
void ResetISR(void) {
    ...
    volatile unsigned int *__iomux_gpr16_adr = (unsigned int *)0x400AC040; //GPR16 General Purpose Register(IOMUXC_GPR_GPR16)  FLEXRAM_BANK_CFG_SEL: 0 use fuse value to config, 1 use FLEXRAM_BANK_CFG to config
    volatile unsigned int *__iomux_gpr17_adr = (unsigned int *)0x400AC044; //GPR17 General Purpose Register(IOMUXC_GPR_GPR17)  FLEXRAM_BANK_CFG: bank 0~15
    const unsigned int __flexram_bank_cfg    = 0x5AAFFAA5;             //128k OCRAM, 256k DTCM, 128k ITCM write to FLEXRAM_BANK_CFG

    __asm volatile ("dsb");
    __asm volatile ("isb");

    *__iomux_gpr17_adr = __flexram_bank_cfg; // write FLEXRAM_BANK_CFG
    
    __asm volatile ("dsb");
    __asm volatile ("isb");

    *__iomux_gpr16_adr |= 0x04; // set FLEXRAM_BANK_CFG_SEL to use FLEXRAM_BANK_CFG
