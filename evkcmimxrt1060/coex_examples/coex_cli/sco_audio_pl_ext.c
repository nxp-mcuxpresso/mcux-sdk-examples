...
#define MAX_CODEC_INIT_RETRIES 3
...
static void Init_Board_Sco_Audio(uint32_t samplingRate, UCHAR bitWidth)
{
    uint32_t src_clk_hz;
    uint8_t i = 0;
    status_t codec_init_status = kStatus_Fail;
    ...
    (defined FSL_FEATURE_PCC_HAS_SAI_DIVIDER && FSL_FEATURE_PCC_HAS_SAI_DIVIDER)
        ...
        /* Add 2 seconds delay to give I2C driver enough time for proper initialization.
         * This is a workaround solution because of a I2C driver issue for EVKC1060
         * platform which causes intermittent failures to codec init sequence and hence
         * resulting in issue in HFP operation*/

        BT_sleep(2);
        /* Codec Init*/
        for(i = 0; i < MAX_CODEC_INIT_RETRIES; i++)
        {
        	codec_init_status = CODEC_Init(&codec_handle, &boardCodecScoConfig);
            if (codec_init_status == kStatus_Success)
            {
                PRINTF("CODEC_Init success:%d\n", i);
                break;
            }
        }
