...
#if !defined(RW612_SERIES) && !defined (MIMXRT1062_SERIES)
...
restart:
...
#elif defined(MIMXRT1062_SERIES)
        ret = LPUART_RTOS_Receive(&ncp_rtos_handle, tlv_buf + tmp_len, TLV_CMD_HEADER_LEN, &rx_len);
#endif
        tmp_len += rx_len;
        total   += rx_len;
    }
    ...
    while (tmp_len != (cmd_len - TLV_CMD_HEADER_LEN + NCP_CHKSUM_LEN))
    {
        ...
        tmp_len += rx_len;
        total   += rx_len;
        if ((ret == kStatus_USART_RxRingBufferOverrun) || total >= TLV_CMD_BUF_SIZE)
        {
