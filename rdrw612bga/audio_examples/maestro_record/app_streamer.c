...
status_t STREAMER_mic_Create(streamer_handle_t *handle, out_sink_t out_sink, char *file_name)
{
...
#if (defined(PLATFORM_RT1170_EVKB) || DEMO_CODEC_CS42448)
    ...
    streamer_set_property(handle->streamer, 0, prop, true);
#endif
#endif // (defined(PLATFORM_RT1170_EVKB) || DEMO_CODEC_CS42448)

#if (defined(PLATFORM_RT1170_EVKB) || defined(MCXN547_cm33_core0_SERIES) || DEMO_CODEC_CS42448)
    prop.prop = PROP_MICROPHONE_SET_BITS_PER_SAMPLE;
    prop.val  = 32;
    streamer_set_property(handle->streamer, 0, prop, true);
#endif

#if (defined(PLATFORM_RW61X))

    prop.prop = PROP_MICROPHONE_SET_NUM_CHANNELS;
    prop.val  = DEMO_DMIC_NUM;
    streamer_set_property(handle->streamer, 0, prop, true);
#endif

    prop.prop = PROP_MICROPHONE_SET_FRAME_MS;
    prop.val  = DEMO_AUDIO_FRAME_MS;
    streamer_set_property(handle->streamer, 0, prop, true);
