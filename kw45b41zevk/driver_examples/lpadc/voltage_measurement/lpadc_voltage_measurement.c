...
#define DEMO_LPADC_USER_CHANNEL     29U
...
#define DEMO_LPADC_REFERENCE_VOLTAGE_MV  1800U
 ...
 * Prototypes
 ******************************************************************************/
uint16_t BOARD_GetVoltage(uint16_t adcValue, uint32_t adcFullRange);
...
uint16_t BOARD_GetVoltage(uint16_t adcValue, uint32_t adcFullRange)
{
    /* The Channel 29 is VDD_LDO_SYS / 4, need to mltiply by 4 in order to get
     * the voltage of VDD_LDO_SYS */
    return DEMO_LPADC_REFERENCE_VOLTAGE_MV * adcValue / adcFullRange << 2;
}
...
int main(void)
{
    ...
    PRINTF("LPADC voltage measurement example\r\n");
     ...
     */
    PRINTF("Please press any key to get user channel's voltage value.\r\n");
    while (1)
    {
        GETCHAR();
        ...
        PRINTF("\r\nVoltage: %d mv\r\n",
               BOARD_GetVoltage((g_LpadcResultConfigStruct.convValue) >> g_LpadcResultShift, g_LpadcFullRange));
