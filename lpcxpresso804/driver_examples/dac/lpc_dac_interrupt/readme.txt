...
and configure the DAC module to make it work in interrupt mode. user-defined waveform array would
be transfered to pre-buffer in order in ISR. Finally, we can probe the signal from DAC_OUT pin
...
## Expected Result
------------------------
Probe the signal from DAC_OUT(CAN8 pin2) pin by using an oscilloscope and see triangular wave.
