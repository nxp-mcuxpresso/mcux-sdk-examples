...
#ifndef ZIGBEE_NB_POLL_BEFORE_SLEEPING 
#define ZIGBEE_NB_POLL_BEFORE_SLEEPING 20
#endif
...
void APP_cbTimerPoll( void *pvParam)
{
    ...
    case E_RUNNING:
    {
        if(!bDisallowSleep)
        {
            ...
            if(u32PollTime > ZIGBEE_NB_POLL_BEFORE_SLEEPING)
            {
...
static void vAppHandleZdoEvents( BDB_tsZpsAfEvent *psZpsAfEvent)
{
    ...
    switch(psAfEvent->eType)
    {
        case ZPS_EVENT_NWK_JOINED_AS_ENDDEVICE:
            DBG_vPrintf(TRACE_APP_EVENT, "APP_EVENT: Joined as end devicet\r\n");
            if(ZTIMER_eStart(u8TimerPoll, POLL_TIME_FAST) != E_ZTIMER_OK)
            {
                DBG_vPrintf(TRACE_APP, "\r\nAPP: Failed to start poll");
            }
