...
J2-8 is ACOMP negative input pin, which can sample external voltage.
...
1. Connect a micro USB cable between the PC host and the CMSIS DAP port(J3 on the 
   LPCXpresso812MAX board) for receiving debug information.
...
## Expected Result
------------------------
If the ACOMP input pin(J2-8) connects the GND, LED(D3) will be turned on.
If the ACOMP input pin(J2-8) connects the 3.3V, LED(D3) will be turned off.
