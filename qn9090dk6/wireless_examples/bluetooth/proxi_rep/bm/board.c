...
#if defined(gResetPinSupport) && (gResetPinSupport)
static  gpioInputPinConfig_t resetPin = {
    .gpioPort = gpioPort_A_c,
    .gpioPin = 20,
    .pullSelect = pinPull_Disabled_c,
...
static void reset_pin_changed(void)
{
    GpioClearPinIntFlag(&resetPin);
    /* Reset the device */
    ResetMCU();
}

void BOARD_ResetPinRegister(void)
{
    GpioInputPinInit(&resetPin, 1);
    GpioInstallIsr(reset_pin_changed, gGpioIsrPrioNormal_c, gGpioDefaultNvicPrio_c >> 1, &resetPin);
}
#endif
